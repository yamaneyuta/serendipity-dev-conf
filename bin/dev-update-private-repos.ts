...
const githubDependencies = Object.entries(packageJson.dependencies).filter(
	([key, value]) => (value as string).startsWith("github:")
);
const githubDevDependencies = Object.entries(packageJson.devDependencies).filter(
	([key, value]) => (value as string).startsWith("github:")
);
...
if(githubDependencies.length > 0) {
	console.log("npm install", githubDependencies.map(([key, value]) => `${key}@${value}`));
	spawn("npm", ["install", ...githubDependencies.map(([key, value]) => `${key}@${value}`)], { stdio: "inherit" });
}
// devDependenciesのパッケージを再インストール
if(githubDevDependencies.length > 0) {
	console.log("npm install", githubDevDependencies.map(([key, value]) => `${key}@${value}`));
	spawn("npm", ["install", ...githubDevDependencies.map(([key, value]) => `${key}@${value}`)], { stdio: "inherit" });
}
